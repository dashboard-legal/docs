...
<ExpansionPanel title="Prerequisite: IdP Metadata URL">
  - Go to your Identity Management Service, and Configure a SAML2.0 IdP (Identity Provider)
  - You will receive the SP (Service Provider) entity ID, metadata URL, and ACS url at a future step. Enter placeholder values here if they are required.
  - Get the URL of the metadata for the IdP, you will require it in the Dashboard Legal Configuration Screen
</ExpansionPanel>

1. Go to DBL and use the account dropdown to go to *Organization Settings > Auth & Security*.
2. Press "Configure IdP"
3. Paste your IdP metadata url and press "create"
4. You will be given the SP entity ID, metadata URL, and ACS url, which you will plug into your Identity Provider configuration
