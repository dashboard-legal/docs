...
This data is accessible based on our built-in user access controls. Simply put, you can only see the chat & checklist data for a client or workstream if you are given permission by that client/workstream administrator.


## Data Residency
We have a couple of deployment options, use the following tables to determine where data lives and who has access.
...
### DBL On-premise via [Reynen Court](./on-premise-deployments)
| Data | Storage System | User Access | DBL Staff Access |
| ----------    | - | -------- | ----------- |
| Emails | Outlook/Exchange| Inbox owner only | ❌ |
| Documents (with DMS) | iManage/NetDocs | If has permission in DMS | ❌ |
| Documents (without DMS) | RC Deployment | If granted access to board by admin | ❌ |
| Boards | RC Deployment | If granted access to board by admin | ❌ |
| Chat | RC Deployment | If granted access to board by admin | ❌ |
| Checklist | RC Deployment | If granted access to board by admin | ❌ |
