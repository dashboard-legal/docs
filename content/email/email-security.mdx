...
Any additional data that you create in Dashboard Legal (clients, workstreams, chats, checklists) are stored in our secure, encrypted database, protected by access controls that the administrator of your account controls.
...
If you choose to, you may add an attachment received in an email to a client workstream. That document is then available to teammates that are a part of that workstream. Access to a workstream is managed by the workstream administrator, and can be granted/revoked any time.

Documents that are stored using Dashboard Legal's document feature are hosted in a protected AWS S3 bucket. These documents are private, and are made available to Dashboard Legal's users based on our role-based access controls.
